...
def nuke_resource(subscription_id, resource_group_name, resource_name, resource_type):
    # Example: Deleting a specific resource in the resource group
    credential = DefaultAzureCredential()
    resource_client = ResourceManagementClient(credential, subscription_id)

    print(f"Nuking resource '{resource_name}' in resource group '{resource_group_name}'...")
    resource_client.resources.begin_delete(resource_group_name, resource_name, resource_type)
    print("Resource nuked successfully!")
...
def main():
    ...
    selected_resource_index = int(input("Enter the number corresponding to the resource you want to nuke: ")) - 1
    selected_resource_name, selected_resource_type = resource_list[selected_resource_index]

    # 3. Nuke the selected resource
    confirm_nuke = input(f"Do you want to nuke the resource '{selected_resource_name}'? (yes/no): ")
    if confirm_nuke.lower() == "yes":
        nuke_resource(subscription_id, resource_group_name, selected_resource_name, selected_resource_type)
    else:
